...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			let s = url.match(/code=pid(.*)pid/);
			let base = url.match(/u=base(.*)base/);
			if (s) {
				// 是授权后的页面,则做opnegid判断 使用code获取pid
				// 换取oid
				let  u
				if(base && base[1]){
					u= 'http://'+ base[1].replace(/\+/g,'.')		
					
				}
				// console.log(u)
				this.formParams.goods_code = s[1];
				this._loadData('refresh',u);
			} 
